...
class Matchmaking(commands.Cog):
        ...
        async def accept_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            if match and interaction.user.id in match["users"]:
                ...
                if match["users"][interaction.user.id]["accepted"] == False:
                    ...
                    await interaction.response.send_message("You have already accepted the match!", ephemeral=True)
    ...
    async def prepare_server(self, match):
        ...
        embed_ready.add_field(name="**IP**", value=f'``{match["region"]}.csrestored.xyz:{str(server_port)}``', inline=True)
